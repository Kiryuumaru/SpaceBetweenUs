...
/// Yolov5 scorer.
/// </summary>
public class YoloScorer2<T> : IDisposable where T : YoloModel
{
    ...
    private Mat ResizeImage(Mat mat)
    {
        ...
        if (mat.Width < _model.Width && mat.Height < _model.Height)
        {
            fillWidth = (_model.Width - mat.Width) / 2;
            fillWidth2 = _model.Width - mat.Width - fillWidth;

            fillHeight = (_model.Height - mat.Height) / 2;
            fillHeight2 = _model.Height - mat.Height - fillHeight;
            Mat dest = new(new OpenCvSharp.Size(_model.Width, _model.Height), mat.Type());
            Cv2.CopyMakeBorder(mat, dest, fillHeight, fillHeight2, fillWidth, fillWidth2, BorderTypes.Constant, null);
            return dest;
        }
        else
        {
            ...
            if (horizontalFill)
            {
                resizedWidth = mat.Width * _model.Height / mat.Height;
    ...
    /// </summary>
    private DenseTensor<float>[] Inference(Mat mat)
    {
        Mat? resized = null;

        if (mat.Width != _model.Width || mat.Height != _model.Height)
        {
            resized = ResizeImage(mat); // fit image size to specified input size
        }

        var inputs = new List<NamedOnnxValue> // add image as onnx input
        {
            NamedOnnxValue.CreateFromTensor("images", ExtractPixels(resized ?? mat))
        };
    ...
    /// </summary>
    public YoloScorer2(string weights, SessionOptions opts = null) : this()
    {
    ...
    /// </summary>
    public YoloScorer2(Stream weights, SessionOptions opts = null) : this()
    {
    ...
    /// </summary>
    public YoloScorer2(byte[] weights, SessionOptions opts = null) : this()
    {
...
internal class OclCaller2 : IDisposable
{
